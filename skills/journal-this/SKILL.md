...
**Choose `tags` from the registry — never invent one (2026-09-17).** The build gate above is the backstop; this is how to not need it. If the samples carry a `tags:` array, assume the tags are **not free-form** and look for the registry *before* writing frontmatter — in a VitePress journal it's typically a file like `.vitepress/topics.ts`, and listing the valid slugs is one command (`grep -o "slug: '[^']*'" .vitepress/topics.ts`). A tag appearing in an older entry is **not proof it's registered** — entries get retagged and the registry is the only source of truth. Pick from the registered slugs; if nothing fits, use the closest one and tell the Operator a new topic may be warranted. Don't coin a tag and register it on your own initiative: a new topic is a content decision that usually drags in a required sibling page, so it's the Operator's call — the "register it in the same commit" path above is for when they've asked for it. (This happened twice in two days: four unregistered tags on one entry, then an invented `bfp-site` on the next — both failed the deploy, and the second time the installed skill was a stale copy that predated the build-gate rule. If `tags` exist and this paragraph is news to you, check that your installed skill matches its source repo.)
